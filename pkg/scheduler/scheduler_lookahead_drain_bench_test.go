...
func drainToEmpty(tb testing.TB, ctx context.Context, qManager *qcache.Manager, scheduler *Scheduler, wg *sync.WaitGroup, maxCycles int, afterCycle func(cycle int)) int {
	tb.Helper()
		...
		for _, arm := range []struct {
			...
			b.Run(fmt.Sprintf("scenario=%s/gate=%s", sc.name, arm.name), func(b *testing.B) {
				...
				for b.Loop() {
					...
					runtime.GC()
					b.StartTimer()
					totalCycles += drainToEmpty(b, ctx, qManager, scheduler, wg, 4*totalWorkloads+8, nil)
					iterations++
				}
	...
	for _, sc := range drainScenarios() {
		t.Run(sc.name, func(t *testing.T) {
			for _, gateOn := range []bool{false, true} {
				...
				cycles := drainToEmpty(t, ctx, qManager, scheduler, wg, 4*len(sc.workloads)+8, afterCycle)
