...
func (s *Scheduler) schedule(ctx context.Context) wait.SpeedSignal {
	...
	for iterator.hasNext() {
		...
		// spend the subtree's deep-admission budget. A solo ClusterQueue's
		// scope contains only itself and heads() already enforces the depth,
		// so there is nothing to interrupt.
		if deepAdmission && e.status == assumed && !e.secondPass && e.clusterQueueSnapshot.HasParent() {
			...
			if admittedPerSubtree[key] >= qcache.FairSharingDeepAdmissionDepth {
				...
				if droppedByInterrupt > 0 {
					interruptedRoots++
					metrics.DeepAdmissionInterrupt(s.roleTracker)
					log.V(3).Info("Skipping remaining entries from root scope after subtree reached deep-admission depth",
						"subtree", key,
	...
	log.V(2).Info("Workload processing done", "duration", s.clock.Since(phaseStartTime), "deepAdmissionInterrupts", interruptedRoots, "droppedEntries", droppedEntries)
...
func (s *Scheduler) processEntry(
	...
	// Second-pass entries hold their quota already; blockAdmission sequences
	// new admissions and must not strip an existing reservation. A delayed-TAS
	// workload sits in WorkloadsNotReady itself, so it would trip the block
	// because of itself and unset its own reservation.
	if !e.secondPass {
...
func (s *Scheduler) updateAssignmentIfNeeded(
	...
	needsRecompute := (fitsCheck == schdcache.FitsCheckNoTAS && features.Enabled(features.TASRecomputeAssignmentWithinSchedulingCycle)) ||
		(deepAdmission && fitsCheck != schdcache.FitsCheckOk && assumedPerCQ[e.ClusterQueue])
	if needsRecompute {
