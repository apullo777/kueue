...
func TestScheduleForFairSharing(t *testing.T) {
	...
	cases := map[string]scheduleTestCase{
		...
		"fair sharing deep admission: interrupt drops a sibling under-share CQ's remaining entry to the heap": {
			enableFairSharing: true,
			featureGates:      map[featuregate.Feature]bool{features.FairSharingDeepAdmission: true},
			additionalClusterQueues: []kueue.ClusterQueue{
		...
		// Deep admission with a preempting front: the under-share CQ's head
		...
		"fair sharing deep admission: preempting front does not trigger the interrupt and the deeper entry returns to the heap": {
			enableFairSharing: true,
			featureGates:      map[featuregate.Feature]bool{features.FairSharingDeepAdmission: true},
			additionalClusterQueues: []kueue.ClusterQueue{
		...
		// fails during processing; with deep admission the CQ's deeper entry is
		...
		"fair sharing deep admission: deeper entry behind a failed front returns to the heap": {
			enableFairSharing: true,
			featureGates:      map[featuregate.Feature]bool{features.FairSharingDeepAdmission: true},
			additionalClusterQueues: []kueue.ClusterQueue{
