...
var _ = ginkgo.Describe("Scheduler", ginkgo.Label("feature:fairsharing"), func() {
	...
	ginkgo.When("Fair sharing deep admission is enabled", func() {
		...
		// frees MORE capacity than the deep-admission depth, the entries the
		...
		ginkgo.It("keeps scheduling dropped heads across cycles when the burst exceeds the deep-admission depth", framework.SlowSpec, func() {
			ginkgo.By("Saturating cq-a with the whole shared pool")
			for range 8 {
				createWorkload("a", "1")
			}
			...
			ginkgo.By("Freeing three units at once (more than the deep-admission depth)")
